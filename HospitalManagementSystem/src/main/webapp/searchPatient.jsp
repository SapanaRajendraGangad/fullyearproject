...
conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital","root","1234");
 if(conn!=null){
	stmt=conn.prepareStatement("select p.pid,p.pname,p.address,p.contact,p.age,p.gender,p.pdate,p.ptype,p.category,d.dname,p.status from patient p inner join patientdoctorjoin pj on p.pid=pj.pid inner join doctor d on d.did=pj.did where p.pname like '%"+name+"%' and d.did=?");
	stmt.setInt(1,did);
	...
	while(rs.next()){
		...
		out.println("<td><a href='AllocateMedicine?pid="+rs.getInt(1)+"&did="+rs.getInt(12)+"'>Add Medicine</td>");
